...
const CanteenDashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const pendingOrdersCount = canteenOrders.filter(o => o.status === OrderStatus.PENDING).length;
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 lg:p-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-800">Painel da Cantina</h1>
        ...
        <Card>
         ...
         <Card className="bg-primary text-white">
          ...
          <Link to="/canteen/scan">
            <Button fullWidth variant="secondary" className="bg-white text-primary hover:bg-gray-100">Escanear QR Code</Button>
          </Link>
        </Card>
      </div>
            ...
            <ul className="divide-y divide-gray-200">
                ...
                <li key={order.id} className="py-3">
                    ...
                    <div className="text-right">
                        ...
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${order.status === OrderStatus.COMPLETED ? 'bg-green-100 text-green-800' : order.status === OrderStatus.PENDING ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>
                            {order.status}
                        </span>
                    </div>
                    </div>
